...
def test_publish_articles_never_emits_a_file_url_without_a_base(tmp_path):
    """A file:// link is dead the moment it leaves the machine that wrote it. With no
    base URL, the article carries no link at all rather than a fake one."""
    events = a_cluster()
    articles = publish_articles(weekly_theses(events, ON), events, tmp_path)
...
def test_the_weekly_links_to_articles_rather_than_inlining_them(tmp_path):
    ...
    # Canonical, extensionless form (see public_report_link): the deployed site
    # serves the .html-less URL as the 200, the .html form as a 307 redirect to it.
    assert link == f"https://example.test/notes/{article_filename(thesis)[:-5]}"
