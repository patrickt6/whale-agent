...
def test_edinet_normalize(fixtures_dir):
    ...
    # EDINET publishes the filer's own English name in `FilerNameInEnglishDEI`, and it is
    # preferred over the Japanese one. This is reporting what the filing says rather than
    # transliterating: nothing here invents a spelling, it just reads the other field.
    assert event.filer_name == "Nomura Securities Co.,Ltd."
...
def test_dataroma_parses_by_column_header(fixtures_dir):
    ...
    assert len(rows) == 2
    assert rows[0]["insider"] == "Sample Dana K"
...
def test_fullwidth_latin_names_are_folded_to_ascii():
    """Not a translation problem: the same string typed in a different character width.

    Several Japanese and Taiwanese filings carry ordinary English company names in
    fullwidth characters. Folding them is readable and invents nothing. Names genuinely
    written in kanji are left exactly as filed.
    """
    from whale_agent.enrichment.entity_resolution import display_name

    fullwidth = "Ｆｉｄｅｌｉｔｙ　Ｍａｎａｇｅｍｅｎｔ"
    assert display_name(fullwidth) == "Fidelity Management"
    assert display_name("野村證券株式会社") == "野村證券株式会社"
