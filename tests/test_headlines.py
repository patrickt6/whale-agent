...
def test_sector_concentration_headline_names_the_sector_not_one_issuer():
    """Regression for the reader-reported bug.

    sector_concentration sums its filer count and dollar figure across many distinct
    issuers in a sector. The headline must name the sector (the pattern subject), not
    whichever issuer's name happens to sit first in the event list -- that produced
    "57 filers put $2.9B into Informetis Co Ltd names", crediting one company with a
    whole sector's total.
    """
    evs = [
        make_event(
            filer_name=f"A{i}",
            issuer_name="Informetis Co Ltd",
            ticker="INF",
    ...
    h = headline_for(_p("sector_concentration", "Technology", evs))
    assert h == "57 insiders put $57.0M into Technology names", h
    assert "Informetis Co Ltd" not in h
