...
if TYPE_CHECKING:
    from whale_agent.enrichment.history_context import HistoryContext


def history_line(ctx: HistoryContext) -> str:
    from whale_agent.enrichment.history_context import history_line as _line
...
def why_it_matters(ev: NormalizedEvent) -> str:
    ...
    if ev.is_routine:
        bits.append("routine, calendar-driven trade")
...
def _row(
    ...
    # of unstated claim the numeric-provenance gate exists to catch for figures. This
    # label is descriptive only -- see `enrichment/routine_classifier.py` -- and never
    # changes the ranking above.
    lines.append(f"   Trading pattern: {ev.routine_label or 'unknown'}")
    ctx = history.get(ev.event_id) if history else None
    if ctx is not None:
        lines.append(f"   History: {history_line(ctx)}")
    ...
    if note:
        ...
        # layer or not.
        lines.append("   Skeptic's note: this looks like a routine, calendar-driven trade.")
    return "\n".join(lines)
...
def render_digest(
    ...
    `history` (event_id -> HistoryContext) adds one History line per row. None keeps
    the render byte-identical to the render without the feature.
    """
    on = on or date.today()
    ...
    for tier in (Tier.INSTANT, Tier.NOTABLE, Tier.WEEKLY):
        ...
        lines.append("\n" + _TIER_HEADING[tier])
        for ev in group:
            lines.append(_row(rank, ev, prose, history))
            rank += 1
