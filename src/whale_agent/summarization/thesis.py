...
from whale_agent.enrichment.entity_resolution import display_name
...
def _company_name(pattern: Pattern, events: list[NormalizedEvent]) -> str:
    ...
    return display_name(pattern.subject)
...
def headline_for(pattern: Pattern) -> str:
    ...
    # For most detectors the pattern's subject is the issuer everyone filed on, so the
    # company is read off the events. `filer_across_issuers` is the one that inverts:
    # its subject is the *filer*, and the issuers are the many things it filed on. Reading
    # a company off the events there names one arbitrary holding as though it were the
    # actor, which is how a note about Nomura ended up titled after Tri Chemical.
    if pattern.kind == "filer_across_issuers":
        company = display_name(pattern.subject)
    # sector_concentration is a sector-wide finding: the count and the dollar figure
    # are summed across many distinct issuers in the sector, not one company. The
    # subject is already the sector name, so it is what belongs in "{company} names",
    # not the name of whichever issuer happens to sort first in the event list. Using
    # a single issuer name there produced headlines like "57 filers put $2.9B into
    # Informetis Co Ltd names", crediting one company with a whole sector's total.
    elif pattern.kind == "sector_concentration":
        company = pattern.subject
    else:
...
def slug_for(pattern: Pattern, on: date) -> str:
    ...
    The subject is not always spellable in ASCII. A Japanese filer name strips to nothing,
    and the literal fallback that used to fill the gap gave every such pattern the same
    slug on a given day, so two unrelated notes wrote to one file and the second silently
    replaced the first. When the stripped subject is empty, a short digest of the original
    subject stands in: still deterministic, still stable across regenerations, but
    distinct per subject.
    """
    subject = _SLUG_STRIP.sub("-", pattern.subject.lower()).strip("-")
    if not subject:
        subject = hashlib.sha1(pattern.subject.encode("utf-8")).hexdigest()[:10]
...
def _counter_evidence(pattern: Pattern) -> list[str]:
    ...
    if any(e.is_routine for e in events):
        notes.append(
            "At least one of these is classified routine, meaning it follows a calendar "
            "pattern. The classification is ours, not the filer's."
        )
