...
def allowed_numeric_tokens(events: list[NormalizedEvent]) -> set[str]:
...
def check_unsourced_numbers(text: str, events: list[NormalizedEvent]) -> list[str]:
    ...
    """
    allowed = allowed_numeric_tokens(events)
    unsourced: list[str] = []
...
# Retained for callers outside this task's scope (`jobs/pipeline.py`) that still import
# the pre-rename name. `check_unsourced_numbers` is the name to use in new code; this is
# not deprecated so much as unwritten -- the rename didn't reach every caller in one pass.
find_unsourced_numbers = check_unsourced_numbers


def assert_no_hallucinated_numbers(text: str, events: list[NormalizedEvent]) -> None:
