...
a reader-facing table. There is exactly one `send_digest_email()`, so a check placed
...
def find_egress_violations(
    *bodies: str | None,
    ...
    """
    violations: list[str] = []
    permitted = allow_figures or set()

    for body in bodies:
        for token, magnitude in figures_in(body or ""):
            # A whole 13F portfolio is legitimately larger than any single disclosure:
            # Berkshire reports $263B, and BlackRock's runs into the trillions. The
            # ceiling exists to catch a corrupted vendor field in ONE position, so
            # raising it would blunt exactly the check that caught the $1.6 quadrillion
            # row. Instead the section that computed an aggregate declares it, and only
            # that exact token is exempt.
...
def send_digest_email(
    ...
    `attachment` is an optional (filename, html_bytes) pair -- the full report page,
    attached so the reader has it even while the report site (Cloudflare Pages) is
    down. SMTP only for now: Resend's payload shape is untouched, so a caller asking
    for an attachment while WHALE_EMAIL_PROVIDER=resend gets the email without one
    rather than a crash.
    """
    ...
    violations = find_egress_violations(
        subject, digest, html_body, events=events, allow_figures=allow_figures
    )
    if violations:
    ...
    attachments = [(attachment[0], attachment[1], "html")] if attachment else None
    try:
        if s.email_provider == "resend":
            return send_via_resend(subject, digest, html_body, s)
        return send_via_smtp(subject, digest, html_body, s, attachments=attachments)
