...
from whale_agent.redact import redact, settings_secrets
    ...
    *,
    ...
    try:
        ...
        raise SourceUnavailableError(
            redact(
                f"{url} returned non-JSON body: {exc}",
                settings_secrets(settings or get_settings()),
            )
        ) from exc
        ...
        for attempt in range(s.http_max_retries):
            try:
                ...
                error = SourceUnavailableError(
                    redact(f"{method} {url} -> HTTP {resp.status_code}: {detail}", secrets)
                )
