...
class JapanEdinetAdapter(Adapter):
    ...
    def normalize(self, parsed: dict) -> NormalizedEvent:
        ...
        return NormalizedEvent(
            ...
            # `FilerNameInEnglishDEI` is parsed above and is preferred here: EDINET
            # publishes the filer's own English name, so using it is reporting what the
            # filing says rather than transliterating it ourselves. The Japanese name is
            # the fallback, not the default -- reading it first is how a digest ended up
            # attributing positions to 野村證券株式会社 for an English-language reader.
            filer_name=display_name(
                str(
                    parsed.get("filer_name_en")
                    or parsed.get("filerName")
                    or parsed.get("filer_name_ja")
                    or "UNKNOWN"
                )
            ),
