...
class _Registration:
    ...
    requires: tuple[str, ...]
    kind: str
    build: Callable[[Settings, int, date], SourceSpec]
...
def register_source(
    ...
    """Decorator: register a `Settings, limit, on -> SourceSpec` builder under `name`.
    ...
    def decorator(
        build: Callable[[Settings, int, date], SourceSpec],
    ) -> Callable[[Settings, int, date], SourceSpec]:
...
def _build_sec_form4(s: Settings, limit: int, on: date) -> SourceSpec:
    return _spec(
        "sec_form4",
        "SEC Form 4",
        True,  # edgartools needs no key; only a contact string in User-Agent
        lambda: UsSecForm4Adapter().ingest(limit=limit, form="4", user_agent=s.sec_user_agent),
    )


@register_source("sec_13dg", jurisdiction="us")
def _build_sec_13dg(s: Settings, limit: int, on: date) -> SourceSpec:
    return _spec(
        "sec_13dg",
        "SEC SC 13D/G",
        True,
        lambda: UsSec13DGAdapter().ingest(
            limit=limit, form="SC 13D", user_agent=s.sec_user_agent
        ),
    )


@register_source("fmp_insider", jurisdiction="us", requires=("fmp_api_key",))
def _build_fmp_insider(s: Settings, limit: int, on: date) -> SourceSpec:
    return _spec(
        "fmp_insider",
        "FMP insider trades",
        s.fmp_enabled,
...
def _build_fmp_congress(s: Settings, limit: int, on: date) -> SourceSpec:
    return _spec(
        "fmp_congress",
...
def _build_quiver_congress(s: Settings, limit: int, on: date) -> SourceSpec:
    return _spec(
        "quiver_congress",
...
def _build_taiwan_mops(s: Settings, limit: int, on: date) -> SourceSpec:
    return _spec(
        "taiwan_mops",
        "Taiwan MOPS",
        s.taiwan_enabled,
...
def _build_japan_edinet(s: Settings, limit: int, on: date) -> SourceSpec:
    return _spec(
        "japan_edinet",
        "Japan EDINET",
        s.japan_enabled,
...
def _build_arkham(s: Settings, limit: int, on: date) -> SourceSpec:
    return _spec(
        "arkham",
        "Arkham on-chain",
        s.arkham_enabled,
...
def _build_whale_alert(s: Settings, limit: int, on: date) -> SourceSpec:
    return _spec(
        "whale_alert",
...
def _build_dataroma(s: Settings, limit: int, on: date) -> SourceSpec:
    return _spec(
        "dataroma",
...
def _build_senate_lda(s: Settings, limit: int, on: date) -> SourceSpec:
    return _spec(
        "senate_lda",
...
def _build_usaspending(s: Settings, limit: int, on: date) -> SourceSpec:
    return _spec(
        "usaspending",
...
def _build_quiver_lobbying(s: Settings, limit: int, on: date) -> SourceSpec:
    return _spec(
        "quiver_lobbying",
...
def _build_quiver_gov_contracts(s: Settings, limit: int, on: date) -> SourceSpec:
    return _spec(
...
def build_sources(
    ...
    *,
    ...
    specs = [reg.build(s, limit, on) for reg in _REGISTRY.values()]
