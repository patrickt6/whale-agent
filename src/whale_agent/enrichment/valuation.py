...
def _implausible_price(price: float | None) -> str | None:
    ...
    if price < 0:
        return "per-share price is negative"
...
def value_event(
    ...
    # A stored price of exactly zero means "not learned", not "worth nothing". Grants and
    # exempt option exercises legitimately carry no per-share price, and rows ingested
    # before this distinction existed persisted that absence as 0.0. Reading it as a real
    # figure quarantined them as defective, which silently withheld thousands of genuine
    # filings from the record; a real share price is never 0, so the value can only ever
    # have meant absent. Negative still fails below, because that is a real defect.
    if event.price_used == 0:
