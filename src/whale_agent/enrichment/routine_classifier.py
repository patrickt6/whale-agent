...
a routine penalty so calendar-driven trades do not crowd the brief (PRD section 3).
...
def mark_routine(store, events: list, on: date) -> list:
    """Set `is_routine` on each event from the filer's own trading history.

    `classify_routine` answers a question about a filer, and nothing was asking it. The
    flag it sets is read in seven places, including a `ROUTINE_PENALTY` in scoring and
    two LLM prompts that have therefore been told every trade is opportunistic.

    History is read strictly before `on`, so an event is judged against what preceded it
    rather than against itself.
    """
    from whale_agent.storage.db import filer_key_for
    ...
    for event in events:
        ...
        if key not in cache:
            dates = [
                row["disclosure_date"] for row in store.filer_observations(key, before=on)
            ]
            cache[key] = classify_routine(dates)
        # Only ever sets the flag, never clears it. A filer with too little history is
        # unjudgeable, not proven opportunistic, so a "no" here must not overwrite a
        # flag some other stage had grounds to set.
        if cache[key]:
