...
def probe_fmp(settings: Settings, ticker: str = "AAPL") -> list[Probe]:
    ...
    try:
        ...
        probes.append(Probe("fmp", "historical-price-eod/full", False, redact(exc)[:160]))
    ...
    try:
        ...
        probes.append(Probe("fmp", "profile", False, redact(exc)[:160]))
...
def _probe_adapter(
    ...
    try:
        ...
        return Probe(source, label, False, redact(exc)[:160])
    except Exception as exc:  # an unexpected shape is a finding, not a crash
        return Probe(source, label, False, redact(f"{type(exc).__name__}: {exc}")[:160])
...
def probe_links(settings: Settings, sample: int = 5) -> list[Probe]:
    ...
    try:
        ...
        return [Probe("links", "source_url sample", False, redact(exc)[:160])]
