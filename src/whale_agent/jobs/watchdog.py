...
from whale_agent.monitoring.health import (
    MIN_WINDOW_EVENTS,
    core_sources_stale,
    digest_overdue,
    silent_sources,
...
def gate_report(
    store: Store, *, now: datetime | None = None, on: date | None = None
) -> tuple[str, bool]:
    """Decide whether the weekly may send. Returns (report, blocked).

    Separate from `build_report` on purpose. The watchdog answers "is anything wrong",
    which is a question with many shades. This answers "may the reader be emailed",
    which has exactly two answers and must never be softened by a warning that reads
    like a pass.
    """
    stale = core_sources_stale(store, now=now)
    if not stale:
        # Fresh sources do not imply a collected week. Ask the second question.
        thin = thin_window(store, on=on)
        if thin is not None:
            return (
                "WEEKLY SEND BLOCKED: the reporting window is implausibly thin.\n\n"
                f"  {thin} events in the last 7 days, against a floor of "
                f"{MIN_WINDOW_EVENTS}.\n\n"
                "Every source reports as fresh, so this is a gap in the corpus rather\n"
                "than a dead adapter -- most likely ingest was not running. Backfill\n"
                "the window, then re-run the weekly workflow.",
                True,
            )
        quiet = silent_sources(store, on=on)
        if quiet:
            return (
                "Core sources fresh and the window is populated; the weekly may send.\n"
                f"  Below expected volume: {', '.join(quiet)}",
                False,
            )
        return "Core sources fresh and the window is populated; the weekly may send.", False
    lines = ["WEEKLY SEND BLOCKED: a core source has gone stale.", ""]
    for source in stale:
    ...
    lines.append("")
    lines.append(
        "No email was sent to the reader. Fix ingest, then re-run the weekly workflow."
    )
    return "\n".join(lines), True
...
def main() -> None:
    ...
    ap.add_argument(
        "--gate",
        action="store_true",
        help="core-source freshness gate: alert and exit 1 if the weekly must not send",
    )
    args = ap.parse_args()
    ...
    try:
        if args.gate:
            report, blocked = gate_report(store)
            print(report)
            if blocked:
                alert_operator("whale-agent: weekly send blocked", report, settings)
                raise SystemExit(1)
            return
