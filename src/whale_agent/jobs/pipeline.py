...
def collect_events(
    ...
    for spec in specs:
        ...
        try:
            ...
            log.info("Source %s skipped: %s", spec.name, exc)
...
def render_checked(
    ...
    """
    digest = render_digest(ranked, on, prose, coverage_notes)
    if not validate_provenance:
        return digest
    deterministic_ok = True
    if prose is not None and check_unsourced_numbers(digest, ranked):
        log.warning("Prose failed the provenance gate; falling back to template render")
        digest = render_digest(ranked, on, None, coverage_notes)
        deterministic_ok = False
    assert_no_hallucinated_numbers(digest, ranked)
...
def run_daily_digest(
    ...
    *,
    ...
    if events is None:
        specs = specs if specs is not None else build_sources(s, limit=limit, on=on)
        collected = collect_events(specs, store, s)
    ...
    digest = render_checked(ranked, on, prose, coverage_notes)
    ...
    if deliver:
        result = send_digest_email(
            digest,
            s,
            html=build_digest_html(ranked, on, prose, coverage_notes),
            events=ranked,
        )
...
def build_digest_html(
    ...
    """
    markup = render_digest_html(ranked, on, prose, coverage_notes)
    if check_unsourced_numbers(html_to_text(markup), ranked):
        log.warning("HTML digest failed provenance with prose; rebuilding without it")
        markup = render_digest_html(ranked, on, None, coverage_notes)
    if check_unsourced_numbers(html_to_text(markup), ranked):
        log.error("HTML digest failed provenance without prose; falling back to text render")
        return digest_to_html(render_digest(ranked, on, None, coverage_notes))
    return markup
