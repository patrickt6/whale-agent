...
def _label_issuer(events: list[NormalizedEvent]) -> str:
    """The issuer as it should read, folded once here.

    This label becomes both the pattern's subject and the text of its summary, so folding
    at this single point is what keeps a headline and the sentence under it from
    disagreeing about how a company is spelled.
    """
    return display_name(events[0].issuer_name)
...
# The pipeline ingests US insider transactions and Japanese threshold reports. A US
# Form 4 discloses one insider's trade; a Japanese large shareholding report discloses a
# whole position at a fixed ownership percentage. Summed into one sector total those are
# not the same quantity, and the larger reporting floor wins every dollar ranking. When a
# sector group turns out to be one jurisdiction, that is the first thing about it -- and
# it has to be said by the pattern rather than by the prose, because the deterministic
# article ships whenever the model is unreachable.
_COVERAGE_DOMINANCE = 0.9


def _coverage_caveat(group: list[NormalizedEvent]) -> str:
    """One sentence when a group is effectively a single jurisdiction. Empty otherwise."""
    seen = [
        getattr(e.jurisdiction, "value", e.jurisdiction)
        for e in group
        if getattr(e, "jurisdiction", None)
    ]
    if not seen:
        return ""
    top, count = Counter(seen).most_common(1)[0]
    if count / len(seen) < _COVERAGE_DOMINANCE:
        return ""
    return (
        f" It is also a coverage artifact before it is a finding: {count} of {len(seen)} "
        f"filings here are {top}. This pipeline reads US insider transactions, which "
        "disclose one trade at a time, and Japanese threshold reports, which disclose a "
        "whole position, so the two are not comparable quantities and no other market is "
        "read at all."
    )
...
def detect_filer_across_issuers(events: list[NormalizedEvent]) -> list[Pattern]:
    ...
    for _, group in sorted(_group(_buy_side(events), filer_key).items()):
        ...
        if len(issuers) < MIN_FILER_ISSUERS:
            continue
        name = display_name(group[0].filer_name)
        patterns.append(
            Pattern(
