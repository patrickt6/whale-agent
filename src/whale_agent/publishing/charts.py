...
def render_chart_svg(
    ...
    parts = [
        ...
        # SVG text inherits CSS from the embedding page, including letter-spacing set
        # on unrelated wrapper elements (e.g. the page's uppercase eyebrow labels).
        # A fixed additive letter-spacing stretches a short label like "Evo Fund" far
        # more visibly than a long one, which is why only short names looked broken.
        # Reset explicitly rather than depend on nothing upstream ever setting it.
        f'style="background:{PAPER};display:block;letter-spacing:normal;">',
